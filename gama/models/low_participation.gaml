...
global {
	...
	int nb_participants <- 675;			// total number of participants (default: 0)
	
	// people/doctors parameters
	int max_medicine <- 200;			// number of medication a doctor can hold (default: 20)
	...
	init {
		...
		ask nb_participants among people {
			is_participant <- true;
		}
		ask nb_infected_init among people where (!each.is_immune and each.is_participant){
			is_infected <- true;
		}
...
species people skills:[moving] {		
	...
	reflex stay when: target = nil {
		if flip(proba_people_move) and is_participant {
